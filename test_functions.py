...
class Satellite_direct():
    ...
    def f1(self):
        x1, x2, x3 = self._x[...,0], self._x[...,1], self._x[...,2]
        return (x1**2 + 2*x2 - x3 + 2*self._u21**0.5)
...
class Turbine_feedback(MDA):
    ...
    def r2(self):
        ...
        # print(max(np.log(self._tfail) - (Plm/self._Tbulk - 20)))
        # return np.log(self._tfail) - (Plm/self._Tbulk - 20)
        return np.log(self._tfail) - ((Plm/self._Tbulk - 20)  + 2*(self._Peng/1e7)**2)
    ...
    def r3(self):
        ...
        # return self._Peng - (Fperf*(mdot0 - N*mdot)*Cp*T0*(1+Tg/T0-2*np.sqrt(Tg/T0)) + 0.0001*self._recon**2)
        return self._Peng - (Fperf*(mdot0 - N*mdot)*Cp*T0*(1+Tg/T0-2*np.sqrt(Tg/T0)) + 100*self._tfail**2 + 0.0001*self._recon**2)
    ...
    def _run_OpenMDAO(self, x_input):
        prob = om.Problem(turbineGroup_feedback())
        prob.setup()
