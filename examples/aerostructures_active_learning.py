...
from remdo.problems import Aerostructures
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--base-seed", type=int, default=211, help="Base seed for randomized GP initial designs.")
    parser.add_argument("--acq", default="entropy", help="Acquisition method passed to active_learning_loop.")
    parser.add_argument("--output-dir", type=Path, default=Path("results/aerostructures"), help="Output directory.")
...
def make_tracking_input(seed: int) -> torch.Tensor:
    """Create a fixed aerostructures input vector used for diagnostics."""

    generator = torch.Generator(device=get_device()).manual_seed(seed)
    bounds = tensor([[0.0], [300.0]])
    unit_sample = torch.rand(1, 1, dtype=get_dtype(), device=get_device(), generator=generator)
...
def main() -> None:
    ...
    if context.rank == 0:
        ...
        print(f"Running aerostructures example in {mode} mode.", flush=True)
